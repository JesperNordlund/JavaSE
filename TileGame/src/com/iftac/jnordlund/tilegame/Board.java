	...
	public void setUp() {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < 4; j++) {
				...
				if (t == 0) {
					tiles.add(new Tile(t, true, 200, 200));
					solved.add(new Tile(t, true, 200, 200));

				} else {
					tiles.add(new Tile(t, false, 200, 200));
					solved.add(new Tile(t, false, 200, 200));
				}
	...
	public void printBoard() {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < 4; j++) {
				...
				tmp.append(String.format("%02d", getTiles().get(t).getValue()) + "  ");
		...
		System.out.println(tmp.toString().replaceFirst("00", "  "));

		checkIfSolved();

		if (isSolved) {
			System.out.println("Congratulations! You solved the board in " + counter + " moves.");
		}
